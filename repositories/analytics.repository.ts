...
export async function getDashboardBase(todayFrom: string, todayTo: string, topFrom: string, topTo: string, chartFrom: string, chartTo: string) {
  ...
  return {
    omzetToday: await scalar<number>(database, "SELECT COALESCE(SUM(total),0) FROM transactions WHERE paidAt BETWEEN ? AND ? AND status='PAID'", [todayFrom, todayTo]) ?? 0,
    transactionCount: await scalar<number>(database, "SELECT COUNT(*) FROM transactions WHERE paidAt BETWEEN ? AND ? AND status='PAID'", [todayFrom, todayTo]) ?? 0,
    ...
    cashPayment: await scalar<number>(database, "SELECT COALESCE(SUM(total),0) FROM transactions WHERE paidAt BETWEEN ? AND ? AND status='PAID' AND paymentMethod='TUNAI'", [todayFrom, todayTo]) ?? 0,
    qrisPayment: await scalar<number>(database, "SELECT COALESCE(SUM(total),0) FROM transactions WHERE paidAt BETWEEN ? AND ? AND status='PAID' AND paymentMethod='QRIS'", [todayFrom, todayTo]) ?? 0,
    paidTransactions: await rows<any>(database, "SELECT paidAt, total, paymentMethod FROM transactions WHERE status='PAID' AND paidAt BETWEEN ? AND ? ORDER BY paidAt ASC", [chartFrom, chartTo]),
